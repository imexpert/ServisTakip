    ...
    public class CustomerRepository : EntityRepositoryBase<Customer, ProjectDbContext>, ICustomerRepository
    {
        ...
        public async Task<PagedResult<List<CustomerReportDto>>> GetMusteriRaporQuery(MusteriRaporFilter filter, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in result)
            {
                ...
                //var contracts = await Context.Contracts.Where(s => s.DeviceId == item.DeviceId)
                //    .ToListAsync(cancellationToken);

                //var lastContract = contracts.MaxBy(s => s.EndDate);
                //customer.SozlesmeKodu = lastContract?.ContractCode;
                //customer.BaslangicTarihi = lastContract?.StartDate.ToString("dd/MM/yyyy");
                //customer.BitisTarihi = lastContract?.EndDate.ToString("dd/MM/yyyy");

                //if (filter.SozlesmeKodlari.Count > 0 )
                //{
                //    if (filter.SozlesmeKodlari.Any(s=>s == lastContract.ContractCode))
                //    {
                //        list.Add(customer);
                //    }
                //}
