  ...
  methods: {
    ...
    async getCityList() {
      await this.store
        ...
        .then(result => {
          if (result.isSuccess) {
            this.cityList = Array.from({ length: result.data.length }).map((_, idx) => ({
              value: result.data[idx].id,
              label: `${result.data[idx].name}`,
            }));
          }
        })
        .catch(() => {
    ...
    async onCityChange() {
      ...
      if (this.customerData.cityId.toString() !== '') {
        await this.store
          .dispatch(Actions.GET_DISTRICT_LIST, this.customerData.cityId)
          .then(result => {
            if (result.isSuccess) {
              this.districtList = Array.from({ length: result.data.length }).map((_, idx) => ({
                value: result.data[idx].id,
                label: `${result.data[idx].name}`,
              }));
    ...
    async onDistrictChange() {
      await this.store
        ...
        .then(result => {
          if (result.isSuccess) {
            this.customerData.querterId = '';
            this.querterList = Array.from({ length: result.data.length }).map((_, idx) => ({
              value: result.data[idx].id,
              label: `${result.data[idx].name}`,
            }));
          }
